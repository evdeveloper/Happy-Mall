...
class ModelExtensionFeedYandexSitemap extends Model {
    ...
    public function getProducts() {
        $query = $this->db->queryFetchable("SELECT p.product_id, p.date_modified FROM " . DB_PREFIX . "product p 
            LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)
            WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.no_index = 0
